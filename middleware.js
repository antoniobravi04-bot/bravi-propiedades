...
export default async function middleware(request) {
  ...
  let prop = null;
  try {
    const propsRes = await fetch(`${url.protocol}//${url.host}/properties.json`, {
      headers: { 'Cache-Control': 'max-age=240' }
    });
    const props = await propsRes.json();
    prop = props.find(p => String(p.id) === String(propId));
  } catch (e) {
    return; // Si falla la carga, dejar pasar sin modificar
  }

  if (!prop) return;

  // Obtener el HTML estático original
  const pageRes = await fetch(`${url.protocol}//${url.host}/propiedad.html`);
  ...
  const desc    = [prop.tipo, prop.tipoProp, prop.precio, prop.ciudad || 'General Rodríguez']
                    .filter(Boolean).join(' — ');
  const imagen  = prop.imagen || `${url.protocol}//${url.host}/hero-bg-1.jpg`;
...
export const config = { matcher: '/propiedad.html' };
