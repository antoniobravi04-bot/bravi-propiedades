...
// Netlify Image CDN — redimensiona y convierte a WebP automáticamente
function netlifyImg(url, w, h) {
  if (!url || url.startsWith('data:')) return url;
  let q = '/.netlify/images?url=' + encodeURIComponent(url) + '&w=' + w + '&fit=cover&fm=webp&q=82';
  if (h) q += '&h=' + h;
  return q;
}
...
function renderCards(props) {
  ...
  grid.innerHTML = props.map(p => {
    ...
    const img0      = netlifyImg((p.imagenes && p.imagenes[0]) || p.imagen || FALLBACK_IMG, 600, 450);
  ...
  props.forEach((p, i) => {
    ...
    card.querySelector('.card-slide-btn--prev').addEventListener('click', e => {
      ...
      imgEl.src = netlifyImg(imgs[cur], 600, 450);
      imgLink.href = imgLink.href;
